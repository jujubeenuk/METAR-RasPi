...
class FlightRulesLayout:
    ...
    def __getitem__(self, key: str) -> tuple[ColorT, int]:
        # FIX: Convert the flight rule key (e.g., "VFR") to lowercase (e.g., "vfr")
        # to correctly match the class's attribute names (vfr, mvfr, ifr, lifr, na).
        return getattr(self, key.lower(), self.na)
